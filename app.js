...
// ==================================// 1. Carregar cache local IMEDIATAMENTE (Offline-First)
loadLocal();
...
updateHomeButtons();
updatePrizeDisplay();
showView(state.user ? (state.user.role === 'manager' ? 'mgr' : 'emp') : 'login');

// 4. FIREBASE REAL-TIME SYNC
document.addEventListener('DOMContentLoaded', () => {
    if (db) {
        console.log("🔥 Firebase Online - Sincronizando...");
        
        // Sincronismo de Prêmios
        ...
        // Sincronismo de Funcionários (Tempo Real)
        db.ref('employees').on('value', snap => {
            const val = snap.val() || {};
            
            // PROTEÇÃO: Mantém o usuário logado mesmo se o ID mudar durante o sync
            state.employees = val;
            saveLocal();
            renderAll();
            updateHomeButtons();
            
            // Garante que o Dash do funcionário atualize se ele acabou de cadastrar
