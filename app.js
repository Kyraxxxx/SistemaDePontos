...
window.actionEmployeeAuth = () => {
    ...
    if (btn) { btn.innerHTML = "<i class='fa-solid fa-spinner fa-spin'></i> Verificando..."; btn.disabled = true; }

    const resetBtn = () => { if (btn) { btn.innerHTML = origText; btn.disabled = false; } };

    if (!db) { alert("Sem conexão com o servidor!"); resetBtn(); return; }

    db.ref('employees').orderByChild('name').once('value', snap => {
        const all = snap.val() || {};
        ...
        if (fbFound) {
            ...
            if (emp.password === pass) {
                state.employees[id] = emp; // Atualiza local
                loginAs(id, 'employee');
            } else {
                alert("Senha incorreta!");
            }
        ...
        // Novo cadastro
        if (btn) { btn.innerHTML = "<i class='fa-solid fa-spinner fa-spin'></i> Cadastrando..."; }
        ...
        db.ref('employees').push(data)
            .then(ref => {
                ...
                addDevLog('CADASTRO', `Novo: ${name}`);
            })
            .catch(err => { alert("Erro ao salvar: " + err.message); })
            .finally(resetBtn);
    }).catch(err => {
        alert("Erro ao verificar conta: " + err.message);
        resetBtn();
    });
};
