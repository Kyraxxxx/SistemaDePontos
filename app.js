...
try {
    ...
    db.goOnline(); // Força o banco a conectar agora
} catch (e) {
...
window.onload = () => {
    ...
    // Se já estiver logado, vai direto pra tela dele
    if (state.user) {
        addDevLog('LOGIN', `Role: ${state.user.role}`);
        updatePresence(); 
        showView(state.user.role === 'manager' ? 'mgr' : (state.user.role === 'developer' ? 'developer' : 'emp'));
    } else {
        showView('login');
    }
...
function showView(viewName) {
    const idMap = {
        ...
        'employee': 'screen-employee', 'manager': 'screen-manager', 'developer': 'screen-developer'
    };
...
window.actionEmployeeAuth = () => {
    ...
    if (empEntry) {
        ...
        // NOVO CADASTRO (Lógica Síncrona -> Assíncrona)
        ...
        if (db) {
            ...
            newRef.set(data).then(() => {
                ...
                // Garante que o objeto local tenha o dado antes de logar
                state.employees[realId] = data; 
                loginAs(realId, 'employee');
            ...
            // Fallback Offline se o Firebase falhar
...
window.handlePoint = (id, type, val) => {
    ...
    if (db) {
        ...
        addDevLog('ALTERAÇÃO PONTOS', `${type}: ${old} -> ${newValue} para ${emp.name}`);
    }
};
...
window.actionExecuteReset = () => {
    ...
    addDevLog('RESET GLOBAL', `Novo prêmio: ${newPrize}`);
...
function addDevLog(action, details) {
    if (!db) return;
    db.ref('dev_logs').push({
        action,
        details,
        user: state.user ? (state.employees[state.user.id]?.name || state.user.id) : "Desconhecido",
        timestamp: Date.now()
    });
}
...
function renderDeveloperPanel() {
    ...
    db.ref('presence').on('value', snap => {
        ...
        list.innerHTML = Object.values(online).map(p => `
            <div class="dev-log-item">
                ...
                <span class="log-details" style="font-size: 0.7rem;">${p.ua || 'Desconhecido'}</span>
            </div>
        `).join('') || '<div class="dev-log-item">Nenhum dispositivo ativo.</div>';
    });
    ...
    db.ref('dev_logs').limitToLast(50).on('value', snap => {
        ...
        `).join('') || '<div class="dev-log-item">Sem logs registrados.</div>';
    });
}
